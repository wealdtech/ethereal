...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *ResolverContractContenthashChangedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(ResolverContractContenthashChanged)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *ResolverContractContenthashChangedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *ResolverContractContenthashChangedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}
...
func (_ResolverContract *ResolverContractFilterer) FilterContenthashChanged(opts *bind.FilterOpts, node [][32]byte) (*ResolverContractContenthashChangedIterator, error) {
	...
	logs, sub, err := _ResolverContract.contract.FilterLogs(opts, "ContenthashChanged", nodeRule)
	if err != nil {
		return nil, err
	}
	return &ResolverContractContenthashChangedIterator{contract: _ResolverContract.contract, event: "ContenthashChanged", logs: logs, sub: sub}, nil
}
...
func (_ResolverContract *ResolverContractFilterer) WatchContenthashChanged(opts *bind.WatchOpts, sink chan<- *ResolverContractContenthashChanged, node [][32]byte) (event.Subscription, error) {
	...
	logs, sub, err := _ResolverContract.contract.WatchLogs(opts, "ContenthashChanged", nodeRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
...
func (_ResolverContract *ResolverContractFilterer) FilterTextChanged(opts *bind.FilterOpts, node [][32]byte) (*ResolverContractTextChangedIterator, error) {
	...
	logs, sub, err := _ResolverContract.contract.FilterLogs(opts, "TextChanged", nodeRule)
	if err != nil {
		return nil, err
	}
...
func (_ResolverContract *ResolverContractFilterer) WatchTextChanged(opts *bind.WatchOpts, sink chan<- *ResolverContractTextChanged, node [][32]byte) (event.Subscription, error) {
	...
	logs, sub, err := _ResolverContract.contract.WatchLogs(opts, "TextChanged", nodeRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
