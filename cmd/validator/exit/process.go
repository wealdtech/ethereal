...
func (c *command) process(ctx context.Context) error {
	...
	if err != nil {
		return errors.Join(errors.New("failed to initiate validator exit"), err)
	}
...
func (c *command) runSafetyChecks(ctx context.Context,
	...
	if validatorInfo.Validator.WithdrawalCredentials[0] == byte(0) {
		return errors.New("cannot exit a validator with type 0 credentials")
	}
	...
	if validatorActiveEpochs < phase0.Epoch(c.shardCommitteePeriod) {
		return fmt.Errorf("validator has only been active for %d epochs; needs to be active for %d epochs before exiting",
