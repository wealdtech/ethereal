...
func (c *command) runSafetyChecks(ctx context.Context,
	...
	sourceValidatorInfo, found, err := util.ConsensusValidatorInfo(ctx, c.consensusClient, c.sourceValidator)
	if err != nil {
		return err
	}
	if !found {
	...
	if sourceValidatorInfo.Validator == nil {
		return errors.New("source validator is not yet active")
	}

	if sourceValidatorInfo.Status != apiv1.ValidatorStateActiveOngoing {
		return fmt.Errorf("source validator is in state %s; must be in state %s to compound",
			sourceValidatorInfo.Status,
			apiv1.ValidatorStateActiveOngoing,
		)
	}

	if sourceValidatorInfo.Validator.WithdrawalCredentials[0] != byte(1) &&
		sourceValidatorInfo.Validator.WithdrawalCredentials[0] != byte(2) {
		return fmt.Errorf("source validator has type %d credentials; can only consolidate from a validator with type 1 or type 2 credentials",
			int(sourceValidatorInfo.Validator.WithdrawalCredentials[0]),
		)
	}

	if !bytes.Equal(sourceValidatorInfo.Validator.WithdrawalCredentials[12:], from.Bytes()) {
		return errors.New("transaction sender address does not match that of the source validator")
	}

	targetValidatorInfo, found, err := util.ConsensusValidatorInfo(ctx, c.consensusClient, c.targetValidator)
	if err != nil {
		return err
	}
	if !found {
	...
	if targetValidatorInfo.Validator == nil {
		return errors.New("target validator is not yet active")
	}

	if targetValidatorInfo.Status != apiv1.ValidatorStateActiveOngoing {
		return fmt.Errorf("target validator is in state %s; must be in state %s to compound",
			sourceValidatorInfo.Status,
			apiv1.ValidatorStateActiveOngoing,
		)
	}

	if targetValidatorInfo.Validator.WithdrawalCredentials[0] != byte(2) {
		return fmt.Errorf("target validator has type %d credentials; can only consolidate to a validator with type 2 credentials",
			int(sourceValidatorInfo.Validator.WithdrawalCredentials[0]),
		)
	}

	if bytes.Equal(sourceValidatorInfo.Validator.PublicKey[:], targetValidatorInfo.Validator.PublicKey[:]) {
		return errors.New("source and target validator must be different")
	}
