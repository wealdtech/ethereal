...
The depositdata.json file can be generated by ethdo.
...
This will return an exit status of 0 if the transaction is successfully submitted (and mined if --wait is supplied), 1 if the transaction is not successfully submitted, and 2 if the transaction is successfully submitted but not mined within the supplied time limit.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		opts.Value = new(big.Int).Mul(new(big.Int).SetUint64(depositData.Value), big.NewInt(1000000000))

		depositPubKey, err := hex.DecodeString(depositData.PublicKey)
		cli.ErrCheck(err, quiet, "Failed to parse deposit public key")
		depositWithdrawalCredentials, err := hex.DecodeString(depositData.WithdrawalCredentials)
		cli.ErrCheck(err, quiet, "Failed to parse deposit withdrawal credentials")
		depositSignature, err := hex.DecodeString(depositData.Signature)
		cli.ErrCheck(err, quiet, "Failed to parse deposit signature")
		depositDataRootTmp, err := hex.DecodeString(depositData.DepositDataRoot)
		cli.ErrCheck(err, quiet, "Failed to parse deposit data root")
		var depositDataRoot [32]byte
		copy(depositDataRoot[:], depositDataRootTmp)
		signedTx, err := contract.Deposit(opts, depositPubKey, depositWithdrawalCredentials, depositSignature, depositDataRoot)
		...
		handleSubmittedTransaction(signedTx, log.Fields{
			...
			"depositWithdrawalCredentials": depositWithdrawalCredentials,
...
func init() {
	...
	beaconDepositCmd.Flags().StringVar(&beaconDepositData, "data", "", "The data to set, provided by ethdo or a similar command")
